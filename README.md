...
An interactive **Power BI Dashboard** built on ball-by-ball and match-level IPL data covering all seasons from **2008 to 2025**. The dashboard provides dynamic, season-wise insights into match results, player awards, team performance, and boundary statistics.
...
|------|-------------|-------------|
| `ball_by_ball_data.csv` | Every delivery bowled across all IPL seasons | match_id, batter, bowler, batter_runs, is_wicket, wicket_kind, team_batting, team_bowling |
| `ipl_matches_data.csv` | Match-level results and metadata | match_id, season, match_date, team1, team2, match_winner, match_type |
...
### 🏆 Season Overview
- **Season Slicer** — Filter entire dashboard by any IPL season (2008–2025)
- **Champion** — Winning team of the selected season
- **Runner Up** — Finalist team of the selected season
- **Season Year Badge** — Dynamic year display
...
- Player photo, name, total runs, and team name

### 🟣 Purple Cap
- Top wicket-taker of the season (bowler wickets only)
- Player photo, name, wicket count, and team name

### 🏏 Max Boundary — Fours
- Player with the most fours in the season
- Player photo, name, four count, and team name

### 💥 Max Boundary — Sixes
- Player with the most sixes in the season
- Player photo, name, six count, and team name

### 📋 Points Table
- Team-wise **Matches Played** and **Matches Won**
- Team logos displayed alongside stats
...
- `USERELATIONSHIP` used for Points Table (team1 & team2 separate columns)

### Image Integration
- Player and team images use **publicly hosted URLs** (documents.iplt20.com)
- `image_url` and `player_image` columns set to **Data Category: Image URL** in Modeling tab
- Images render natively inside Power BI Table and Card visuals

### Purple Cap — Special Filter
...
![CSV](https://img.shields.io/badge/CSV-Data-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)
...
- The 4 CSV files placed in the correct data source path
